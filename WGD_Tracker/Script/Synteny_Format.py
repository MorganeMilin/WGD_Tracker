        ...
        if not line.startswith('#'):
            ...
            if line[2] == motif:
                ...
                if (start, end) not in dico_sp[species][chrom]:
                    ...
                    gene_db_count[species] = gene_db_count[species] + 1 if species in gene_db_count else 1
                    if type(dico_sp[species][chrom][(start, end)]) is not list:
                        raise ValueError(f"On s'attend a avoir une liste alors que ici on a {type(dico_sp[species][chrom][(start, end)])}")
...
for species in [sp1, sp2]:
    for chrom in sorted(dico_sp[species]):
        ...
        for position in sorted(dico_sp[species][chrom]):
            if type(dico_sp[species][chrom][position]) is not list:
...
########
# Data # Ajouter une correction des nom !!! 
########
print('dict_gff:', dict_gff)
...
for line in inputfile:
    ...
    sp1_gene, sp2_gene = line[0], line[1]
    print('line:', line)
    print('sp1_gene:', sp1_gene, 'sp2_gene:', sp2_gene)
    print()
    if sp1_gene in dict_gff[sp1] and sp2_gene in dict_gff[sp2]:
        sp1_chrom, sp1_nb = dict_gff[sp1][sp1_gene][0], dict_gff[sp1][sp1_gene][1]
        sp2_chrom, sp2_nb = dict_gff[sp2][sp2_gene][0], dict_gff[sp2][sp2_gene][1]
        fct.prep_data(chrom=(sp1_chrom, sp2_chrom), nb=[sp1_nb, sp2_nb], infos=[sp1_gene, sp2_gene, float(line[3]), float(line[5])], dico=dict_data1)
        fct.prep_data(chrom=(sp1_chrom, sp2_chrom), nb=[sp2_nb, sp1_nb], infos=[sp1_gene, sp2_gene, float(line[3]), float(line[5])], dico=dict_data2)
    else:
        raise ValueError(f"That's not possible !!! check gff for the following gene: {sp1_gene, sp2_gene}")
inputfile.close()
